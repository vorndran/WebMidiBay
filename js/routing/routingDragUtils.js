...
import { addClass, hasClass, removeClassFromAll } from '../html/domUtils.js';
...
function initializeDragLineIfNeeded(event) {
  if (!midiBay.graphTag.dragLine && midiBay.clickedInPortTag) {
    addClass(midiBay.clickedInPortTag, 'routing_source');
...
function updateHoverTargetClasses(event_x, event_y) {
  ...
  if (hoveredTag && hasClass(hoveredTag, 'output')) {
...
function preventSelect() {
  // Drag-spezifische Listener bleiben lokal für sauberes Auf- und Abbauen
